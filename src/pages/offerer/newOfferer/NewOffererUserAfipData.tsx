...
function NewOffererUserAfipData() {
  const classes = NewOffererAfipDataStyles();

  const [error, setError] = useState<string>();
  const { confirmedUserData, setLoading, nosisUserData, setNosisUserData } =
    ...
    <form id="assignNewRoleAfipData-form" onSubmit={handleSubmit(onSubmit)}>
      ...
      >
        ...
        <ControlledTextFieldFilled
          ...
          fullWidth
          disabled={!!nosisUserData && !confirmedUserData}
          className={clsx('', {
            [classes.opacity]: !!nosisUserData,
          })}
        />

        <DisclaimerNosis opacity={!!nosisUserData} />
...
export default NewOffererUserAfipData;
