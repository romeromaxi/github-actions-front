...
interface CompanyLibraryFlowProps {
  companyId: number;
  flowSection:
    | Sections.PostClosingMovementsCompanyLegal
    | Sections.PostClosingMovementsCompanyPhysical;
}

function CompanyLibraryFlow({
  companyId,
  flowSection,
}: CompanyLibraryFlowProps) {
  ...
  useEffect(() => {
    Promise.all([
      HttpCacheFiles.getTypesBySection(flowSection),
  ...
  const searchDocuments = useCallback(() => {
    ...
    HttpFilesCompanySearch.searchFlows(companyId, libraryFilter)
      .then((r) => {
        setFilesCompany(r);
        setLoading(false);
...
export default CompanyLibraryFlow;
