...
interface CompanyLibraryFinancePhysicalPersonProps {
  companyId: number;
}

function CompanyLibraryFinancePhysicalPerson({
  companyId,
}: CompanyLibraryFinancePhysicalPersonProps) {
  const [filesCompany, setFilesCompany] =
    useState<EntityListWithPagination<StatementsFile>>();
  ...
  const columns: ITableColumn[] = [
    ...
    {
      label: 'Año Ejercicio',
      onRenderCell: (file: StatementsFile) => (
        <Typography>
          {dateFormatter.toYearDate(file[StatementsFileFields.Date])}
        </Typography>
      ),
    },
  ...
  const searchDocuments = useCallback(() => {
    ...
    setLoading(true);
    HttpFilesCompanySearch.searchStatements(companyId, libraryFilter)
      .then((r) => {
        setFilesCompany(r);
        setLoading(false);
...
export default CompanyLibraryFinancePhysicalPerson;
