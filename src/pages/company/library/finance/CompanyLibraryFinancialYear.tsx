...
import { HttpCompanyFinance, HttpFileDocument } from '../../../../http';
...
function CompanyLibraryFinancialYear({ companyId }: { companyId: number }) {
  const [filesCompany, setFilesCompany] =
    useState<EntityListWithPagination<FinancialYearDocument>>();
  const [error, setError] = useState<boolean>(false);
  const [loading, setLoading] = useState<boolean>(true);
  const [libraryFilter, setLibraryFilter] = useState<FinancialYearFileFilter>({
  ...
  const { control, handleSubmit, reset, getValues, setValue, watch } =
    useForm<FinancialYearFileFilter>({
      defaultValues: {
        ...libraryFilter,
      },
    });
  ...
  useEffect(() => {
    Promise.all([
      HttpCacheFiles.getTypesBySection(Sections.FinancialYear),
  ...
  const columns: ITableColumn[] = [
    ...
    { label: 'Año Ejercicio', value: FinancialYearDocumentFields.Year },
  ...
  const searchDocuments = useCallback(() => {
    ...
    setLoading(true);
    HttpFilesCompanySearch.searchExercises(companyId, libraryFilter)
      .then((r) => {
        setFilesCompany(r);
        setLoading(false);
  ...
  const onSubmit = (data: FinancialYearFileFilter) => {
    setLibraryFilter({ ...data });
    setShouldSearch(true);
  };

  const loadPossibleYears = useCallback(() => {
    return HttpCompanyFinance.getYears(companyId);
  }, []);
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            <Grid spacing={3} direction="row" alignItems="center" container>
              <Grid item xs={2}>
                <AsyncSelect
                  label={'Año Ejercicio'}
                  loadOptions={loadPossibleYears}
                  control={control}
                  fullWidth
                  name={FinancialYearDocumentFields.Year}
                />
              </Grid>
              <Grid item xs={4}>
                <ControlledTextFieldFilled
...
export default CompanyLibraryFinancialYear;
