...
interface CompanyLibraryBureauProps {
  companyId: number;
}

enum BureauFileFilterFields {
  PersonId = 'idPersona',
}

interface BureauFileFilter extends EntityFilterPagination {
  [BureauFileFilterFields.PersonId]?: number;
}
...
const CompanyLibraryBureau = ({ companyId }: CompanyLibraryBureauProps) => {
  const [error, setError] = useState<boolean>(false);
  const [loading, setLoading] = useState<boolean>(true);
  const [filesCompany, setFilesCompany] =
    useState<EntityListWithPagination<Document>>();
  const [libraryFilter, setLibraryFilter] = useState<BureauFileFilter>({
    [EntityPaginationFields.PageSize]: 10,
    [EntityPaginationFields.ActualPage]: 1,
  });
  const [shouldSearch, setShouldSearch] = useState<boolean>(true);

  const { control, handleSubmit } = useForm<BureauFileFilter>({
    defaultValues: {
      ...libraryFilter,
    },
  });
      ...
      .then((r) => {
        const files: EntityListWithPagination<Document> = {
          [EntityListWithPaginationFields.List]: r.filter(
            (x) => x[FileBaseFields.FileSectionCode] === Sections.PublicBases,
          ),
  ...
  const onSubmit = (data: BureauFileFilter) => {
            ...
            >
              ...
              <Grid item xs={4}>
                <ControlledTextFieldFilled
                  label="Persona"
                  control={control}
                  name={BureauFileFilterFields.PersonId}
                  fullWidth
                />
              </Grid>
              <Grid item xs={2}>
...
export default CompanyLibraryBureau;
