...
function MyCompanyTab({ company, onReload }: MyCompanyTabProps) {
  const isLegalPerson: boolean =
    company[CompanyViewDTOFields.PersonTypeCode] == PersonTypes.Legal;
  const [filesCompany, setFilesCompany] = useState<Document[]>();
  ...
  const section = isLegalPerson ? Sections.CompanyLegal : Sections.CompanyPhysical;
        ...
        <Card sx={{borderRadius: 3}}>
          <CardContent>
            <Stack spacing={3}>
              ...
              <Stack spacing={1}>
                {filesCompany && filesCompany.length > 0 ? (
                  filesCompany.map((file) => (
                    <FileDocumentDetail
