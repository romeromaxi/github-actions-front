...
interface CompanySolicitationByStatusCardProps {
  companyId: number;
}

function CompanySolicitationByStatusCard({
  companyId,
}: CompanySolicitationByStatusCardProps) {
  const [statusData, setStatusData] = useState<SolicitationTotalsView[]>();

  useEffect(() => {
    HttpSolicitation.getCurrentSolicitationsByStatesClassifications(
      companyId,
    ).then((res) => {
      setStatusData(res);
    });
...
export default CompanySolicitationByStatusCard;
